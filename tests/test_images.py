...
from tests.common import (RED_TRUCK_IMAGE_FILE_PATH, TRUCK_IMAGE_URL, both_channels, metadata,
                          raise_on_failure, wait_for_inputs_upload)
...
def test_post_list_patch_get_delete_image(channel):
  ...
  try:
    ...
    list_response = stub.ListInputs(service_pb2.ListInputsRequest(per_page=1), metadata=metadata())
    ...
    get_response = stub.GetInput(service_pb2.GetInputRequest(input_id=input_id), metadata=metadata())
...
def test_post_delete_batch_images(channel):
  ...
  delete_response = stub.DeleteInputs(
    service_pb2.DeleteInputsRequest(ids=[input_id1, input_id2]),
    metadata=metadata()
  )
  raise_on_failure(delete_response)
...
def test_post_patch_get_image_with_id_concepts_geo_and_metadata(channel):
        ...
        resources_pb2.Input(
          ...
          data=resources_pb2.Data(
            ...
            geo=resources_pb2.Geo(
              geo_point=resources_pb2.GeoPoint(longitude=55., latitude=66),
            ),
  ...
  assert inp.data.concepts[0].value == 1.
  assert inp.data.concepts[1].id == "some-negative-concept"
  assert inp.data.concepts[1].value == 0.
  ...
  assert inp.data.geo.geo_point.longitude == 55.
  assert inp.data.geo.geo_point.latitude == 66.
    ...
    service_pb2.PatchInputsRequest(
      ...
      inputs=[
        resources_pb2.Input(
          ...
          data=resources_pb2.Data(
            concepts=[
              resources_pb2.Concept(id="another-positive-concept", value=1)
            ],
            geo=resources_pb2.Geo(
              geo_point=resources_pb2.GeoPoint(longitude=77., latitude=88.)
            ),
  ...
  assert inp.data.geo.geo_point.longitude == 77.
  assert inp.data.geo.geo_point.latitude == 88.
  assert inp.data.metadata["new-key"] == "new-value"
  assert inp.data.metadata["key1"] == 123  # Since we use the merge action, the old values should remain

  delete_response = stub.DeleteInputs(
    service_pb2.DeleteInputsRequest(ids=[input_id]),
    metadata=metadata()
  )
  raise_on_failure(delete_response)
