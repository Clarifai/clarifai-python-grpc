...
def metadata(pat=False):
    if pat:
        return (("authorization", "Key %s" % os.environ.get("CLARIFAI_PAT_KEY_SECURE_DATA_HOSTING")),)
    else:
        return (("authorization", "Key %s" % os.environ.get("CLARIFAI_API_KEY_SECURE_DATA_HOSTING")),)
...
secure_data_hosting_url = os.envrion.get("CLARIFAI_SECURE_DATA_HOSTING_URL", default_secure_data_hosting_url)
########
...
def test_adding_inputs(channel):
    ...
    get_user_response = stub.GetUser(
        service_pb2.GetUserRequest(
           user_app_id=resources_pn2.UserAppIDSet(user_id="me") 
	),
    ...
    post_video_response = stub.PostInputs(
        service_pb2.PostInputsRequest(
		    ...
		    id=input_vid1,
                    data=resources_pb2.Data(
                        video=image=resources_pb2.Video(url=BEER_VIDEO_URL, allow_duplicate_url=True), 
                    )
                )
            ]
        ),
	...
	for inp in inputs:
            ...
            for size in sizes:
                expected_input_url = get_expected_input_url(inp, app_cfid, user_cfid, size, input_type, bytes_data_hashes[inp])
                input_url = os.path.join(get_input_response.input.data.image.hosted.prefix, size, get_input_response.input.data.image.hosted.suffix)
...
def get_expected_input_urls(input_id, app_cfid, user_cfid, size, input_type, filename):
    expected_input_url = "%s/%s/users/%s/apps/%s/inputs/%s/%s" % (secure_data_hosting_url, size, user_cfid, app_cfid, input_type, filename)
