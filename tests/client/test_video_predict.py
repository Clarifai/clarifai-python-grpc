...
def test_predict_video_url_with_custom_sample_ms(channel):
  ...
  request = service_pb2.PostModelOutputsRequest(
    ...
    model_id=GENERAL_MODEL_ID,
    inputs=[
      resources_pb2.Input(data=resources_pb2.Data(video=resources_pb2.Video(url=BEER_VIDEO_URL)))
    ],
    model=resources_pb2.Model(
      output_info=resources_pb2.OutputInfo(
...
def test_predict_video_bytes(channel):
  ...
  request = service_pb2.PostModelOutputsRequest(
    ...
    model_id=GENERAL_MODEL_ID,
    inputs=[
      resources_pb2.Input(data=resources_pb2.Data(video=resources_pb2.Video(base64=file_bytes)))
    ],
  )
  response = post_model_outputs_and_maybe_allow_retries(stub, request, metadata=metadata(pat=True))
  raise_on_failure(response)
